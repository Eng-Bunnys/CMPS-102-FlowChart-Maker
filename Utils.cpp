 ...
 *
 * @function
 * @param {string} string - The input string to be converted to lowercase.
 * @returns {string} The lowercase version of the input string.
 *
 * @example
 * const originalString = "Hello World";
 * const lowercaseString = Utils::toLowerCase(originalString);
 * console.log(lowercaseString); // Output: "hello world"
 */
std::string Utils::toLowerCase(const std::string& string) {
    std::string LowerCaseString;
    for (int i = 0; i < string.length(); i++) {
        LowerCaseString += std::tolower(string[i]);
    }
